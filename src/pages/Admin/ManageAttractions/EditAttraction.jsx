...
import api, { getDestinations, getTrips, getAttractionById, updateAttraction, uploadFile } from '../../../services/api';
import styles from './AddAttraction/AddAttraction.module.css';
...
const EditAttraction = () => {
  ...
  const fetchAttractionDetails = async () => {
    try {
      ...
      // Ensure arrays have at least one empty item if they are empty in DB
      ...
      setFormData({
        ...
        placesToVisitAround: ensureArray(data.placesToVisitAround),
        thingsToDo: ensureArray(data.thingsToDo),
        mustTryFood: ensureArray(data.mustTryFood),
        cultureAndNature: data.cultureAndNature || '',
        localAttractions: ensureArray(data.localAttractions),
        shoppingSightseeing: ensureArray(data.shoppingSightseeing),
        bestTimeToVisit: data.bestTimeToVisit || '',
        faqs: ensureFaq(data.faqs),
        relatedTrips: data.relatedTrips || [],
      });
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert(err.response?.data?.message || 'Failed to update attraction');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h2 className={styles.title}>Edit Attraction</h2>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div className={styles.formActions}>
          <button type="submit" className={styles.submitBtn} disabled={loading || uploadingImage}>
            {loading ? 'Saving...' : 'Update Attraction'}
          </button>
        </div>
      </form>
    </div>
